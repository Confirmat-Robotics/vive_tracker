"""Pydantic models for Vive tracker messages."""
...
class Configuration(BaseModel):
    """Transform and naming configuration shared with Vive tracker server."""
    ...
    name_mappings: dict = Field(default_factory=dict)


class ViveDynamicObjectMessage(BaseModel):
    """Dynamic tracker pose, velocity, and status information."""
    ...
    def __repr__(self) -> str:  # pragma: no cover - repr formatting
        return (
            ...
            f"qx: {round(self.qx, 5)}, qy: {round(self.qy, 5)}, qz: {round(self.qz, 5)}, qw: {round(self.qw, 5)} | "
...
class ViveStaticObjectMessage(BaseModel):
    """Static tracker pose when velocity is not required."""
